...
func pathSum1(root *TreeNode5, targetSum int) [][]int {
	var ans [][]int
	var pathFunc func(*TreeNode5, *[]int, int)
	pathFunc = func(node *TreeNode5, path *[]int, sum int) {
		if nil == node {
			return
		}
		*path = append(*path, node.Val)
		...
		pathFunc(node.Left, path, sum)
		pathFunc(node.Right, path, sum)
		*path = (*path)[:len(*path)-1]
	}
	var tempPath []int
	pathFunc(root, &tempPath, targetSum)
	return ans
}
